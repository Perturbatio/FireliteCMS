...
class NodesPlugin extends FireliteBasePlugin
{
	...
	public function action_add($requested_plugin_details, $view_dir, $tree_id = null, $parent_node_id = null)
	{
		...
		if (Request::method() == 'POST') {
			...
			if ($node_valid === true) {
				...
				//TODO:create a nodetype instance rather than assuming page, then have it handle the editing
				...
				if ($validation_result === true) {
					...
					return Redirect::to( Firelite::getPluginURL( 'nodes', 'edit', array( $node->id ) ));
	...
	public function action_edit($requested_plugin_details, $view_dir, $page_id = 0)
	{
		...
		//echo 'getNodeEditors<pre>', print_r(Firelite::getNodeEditors(), true), '</pre>';
		...
		if (isset($requested_plugin_details['params'][0])) {
			$node_id = $requested_plugin_details['params'][0];
			$node = FireliteNode::find($node_id);
		}

		if ($node) {

			$editor = Firelite::getNodeEditor($node->nodetype->name);

			if (!$editor) {

				return View::make($view_dir . 'error.index', array('errors' => array(
						'No node editor registered for nodetype ' . $node->nodetype->name
					))
				);
			}
		...
		$plugin_response = $editor->execute(
		...
		Event::fire('firelite.plugin.nodes.edit:after', $event_params);
