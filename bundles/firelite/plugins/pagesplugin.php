...
class PagesPlugin extends FireliteBasePlugin {
	
	/**
	 *
	 * @var type 
	 */
	static public $nodetypes = array(
		'Page',
	);
	...
	protected function node_to_jsonable($node){
		$result = new stdClass();
		...
		if ( class_exists( $handler_class ) ){
			...
			$result->handler_id = $handler->id;
			$result->handler_class = $handler_class;
			//assuming all nodes are pages for now
			//TODO: stop assuming this
			$result->edit_url = Firelite::getPluginURL('pages', 'edit', array($handler->id));
		} else {
			$result->handler_id = 0;
	...
	public function action_index($requested_plugin_details, $view_dir){
		...
		if ($tree){
			...
			$nodes = $tree->getNodeMap(function($node) use ($nodeTypePage){
				if ((int)$node->nodetype_id !== (int)$nodeTypePage->id){//nodetype_id is held as a string in the model, need to cast explicitly to int
					return false;
				}
				return true;
			});
			...
			if ( count( $nodes ) > 1 ) {
				...
				foreach($root->children as $child){
					$treeview_nodes[] = $this->node_to_jsonable($child);
				}
			}
		...
		return View::make($view_dir . 'pages.index')
			->with('nodes', $nodes)
			->with('tree', $tree)
	...
	public function action_add( $requested_plugin_details, $view_dir ){
		...
		return View::make( $view_dir . 'pages.add', $data );
	}
	...
	public function action_edit( $requested_plugin_details, $view_dir, $page_id = 0 ){
		...
		$response = Event::until('firelite.plugin.pages.edit:before', $event_params);
		...
		if ( is_a( $response, 'Redirect' ) ){
			...
			return Redirect::to( Firelite::getPluginURL( 'pages', 'index' ) )->with( 'flash_message', 'You are not allowed to edit this page' );
		...
		if ( $page_id < 1 ){
			return Redirect::to( Firelite::getPluginURL('pages','index') );
		}
		...
		if ( $page ){
			...
			return View::make( $view_dir . 'pages.edit', $data);
		} else {
	...
	public function action_update_structure( $requested_plugin_details, $view_dir){
		...
		if ( Request::method() == 'POST' ){
			if ( !empty( $data ) ){
				...
				foreach($new_structure as $child){
					$root->children[] = $child;
				}
				$start_pos = 1;
				...
				echo json_encode(array('result'=>false,'message'=>'No Input'));
				exit;
			}
		} else {
